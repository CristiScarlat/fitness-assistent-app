...
app.get('/filter-terms', (req, res) => {
    ...
    else if (category !== "bodyPart" && category !== "target" && category !== "equipment" && category !== "all") {
        res.statusCode = 400;
        res.json({ error: 'Bad category parameter value, only "bodyPart", "target" and "equipment" is allowed.' });
    }
    else {
        res.statusCode = 200;
